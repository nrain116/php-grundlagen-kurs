...
<body>
    ...
    <main class="container">
        ...
        $diff = time() - $start;
        ?>
        <p><b><?= (floor($diff / 86400)) ?> Tage</b> liegen zwischen heute (<?= date('d.m.Y') ?>) und dem <?= date('d.m.Y', $start) ?>.</p>
        ...
        if (!empty($_POST['datum'])):
            // Teile der Datumseingabe in Array überführen
            $data = explode('-', $_POST['datum']);

            // Prüfen, ob das Datum korrekt ist
            if (count($data) != 3 || !checkdate((int)$data[1], (int)$data[2], (int)$data[0])):
        ?>
                <p><?= htmlspecialchars($_POST['datum']) ?> ist <b>kein</b> korrektes Datum.</p>
            <?php
            else:
            ?>
                <p>Das Datum <?= htmlspecialchars($_POST['datum']) ?> ist korrekt.</p>
            <?php
            endif;
        else:
            ?>
            <p>Kein Datum eingegeben.</p>
        <?php
        endif;
        ?>
    </main>
